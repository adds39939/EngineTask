...
The corresponding *automated* coverage of "does the generator emit valid UniTask code" lives in `tests/EngineTask.UniTask.Tests/`, which references the real Cysharp UniTask NuGet package — it works in a plain .NET 10 host as long as you stick to synchronously-completing awaits (no Unity player-loop scheduling).
